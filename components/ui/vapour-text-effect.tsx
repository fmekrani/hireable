...
const renderCanvas = ({
  ...
  const font = `${framerProps.font?.fontWeight ?? 600} ${fontSize * globalDpr}px ${framerProps.font?.fontFamily ?? "sans-serif"}`;
  const color = parseColor(framerProps.color ?? "rgb(255, 255, 255)");

  let textX;
  const textY = canvas.height / 2;
  const currentText = framerProps.texts[currentTextIndex] || "hireable.ai";
